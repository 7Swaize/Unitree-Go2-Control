...
class TestLidarFilterNode(unittest.TestCase):
    ...
    def setUp(self):
        def create_node():
            with patch('lidar_processor.lidar_decoder_node.Node.__init__', return_value=None):
                ...
                node._config = FilterConfig(**self.config_params)
    ...
    def validate_sor(self, outliers):
        sor_radius = self.node._config.sor_radius
        sor_min_neighbors = self.node._config.sor_min_neighbors
    ...
    def test_c_filter_result(self):
        ...
        # filter outliers
        xyz_data[0] = [self.node._config.min_range - 1, 0, 0]
        xyz_data[1] = [self.node._config.max_range + 1, 0, 0] 
        xyz_data[2] = [10, self.node._config.height_max + 1, 0]
        intensity_data[3] = [self.node._config.intensity_min - 10]
        ...
        with patch.object(LidarFilterNode, 'send_to_bridge', side_effect=self.collect_ndarray):
            self.node.decoded_cloud_callback(msg)

        self.assertTrue(np.all(np.linalg.norm(self.captured_xyz[:, :2], axis=1) >= self.node._config.min_range))
        self.assertTrue(np.all(np.linalg.norm(self.captured_xyz[:, :2], axis=1) <= self.node._config.max_range))
        self.assertTrue(np.all(self.captured_xyz[:, 2] >= self.node._config.height_min))
        self.assertTrue(np.all(self.captured_xyz[:, 2] <= self.node._config.height_max))
        self.assertTrue(np.all(self.captured_intensity >= self.node._config.intensity_min))
